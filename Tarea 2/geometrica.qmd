...
-   a\. ¿Cuál es la probabilidad de necesitar exactamente 5 lanzamientos?
    ...
    k_lanzamientos <- 5     # Número total de lanzamientos (cara en el 5° intento)
    k_fracaso <- k_lanzamientos - 1

    # Probabilidad de necesitar exactamente 5 lanzamientos
    p <- 0.5
    k <- 5
    prob_exacto_5 <- dgeom(k - 1, prob = p)
    cat("a) P(X = 5):", round(prob_exacto_5, 4), "\n")
...
-   c\. Calcule la media y varianza del número de lanzamientos.
    ...
    cat("c) Media:", media_teorica, "\n")
    cat("   Varianza:", varianza_teorica, "\n")
...
-   a\. ¿Cuál es la probabilidad de que anote en el tercer intento por primera vez?
    ...
    p <- 0.7
    k <- 3
    prob_tercer_intento <- dgeom(k - 1, prob = p)
    cat("a) P(X = 3):", round(prob_tercer_intento, 4), "\n")
...
-   b\. ¿Cuántos intentos se espera que necesite para anotar?
    ...
    ```{r}
    media_teorica <- (1 - p) / p
    cat("b) Número esperado de intentos para anotar:", round(media_teorica, 4), "\n")
...
-   c\. ¿Cuál es la probabilidad de necesitar entre 2 y 5 intentos?
    ...
    ```{r}
    prob_entre_2_5 <- sum(dgeom(1:4, prob = p))
...
-   a\. Si se inspeccionan las piezas hasta encontrar la primera defectuosa, ¿cuál es el número esperado de inspecciones?
    ...
    ```{r}
    p <- 0.02
    media_inspecciones <- (1 - p) / p
    cat("a) Número esperado de inspecciones:", round(media_inspecciones, 4), "\n")
...
-   b\. ¿Cuál es la probabilidad de que la primera pieza defectuosa aparezca después de la pieza número 100?
    ...
    ```{r}
    prob_despues_100 <- 1 - pgeom(99, p)
    cat("b) P(X > 100):", round(prob_despues_100, 4), "\n")
...
-   c\. Demuestre la propiedad de falta de memoria para este problema.
    ...
    prob_s_t <- 1 - pgeom(s + t - 1, p)
    prob_s <- 1 - pgeom(s - 1, p)
    prob_t <- 1 - pgeom(t - 1, p)
