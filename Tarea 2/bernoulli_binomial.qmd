...
-   a\. ¿Cuál es la probabilidad de obtener una pieza no defectuosa?
    ...
    ```{r}
    #| label: bernoulli-maquina
    #| code-fold: true

    # Probabilidad de pieza defectuosa
    p_maquina <- 0.02
    p_defecto <- p_maquina
    p_defectuosa <- p_maquina

    # Probabilidad de que una pieza NO sea defectuosa
    p_no_defectuosa <- 1 - p_defectuosa
    prob_no_defecto <- 1 - p_defecto

    cat("a) Probabilidad de pieza no defectuosa:\n")
    cat("   Usando p_defectuosa:", p_no_defectuosa, "\n")
    cat("   Usando p_defecto:", prob_no_defecto, "\n\n")
    ```
...
-   b\. Calcule la media y varianza de esta distribución
    ...
    media_maquina <- p_maquina
    varianza_maquina <- p_maquina * (1 - p_maquina)
    ...
    cat("   Media (Bernoulli):", media_defectuosa, "\n")
    cat("   Varianza (Bernoulli):", varianza_defectuosa, "\n")
    cat("   Media (Binomial):", media, "\n")
    cat("   Varianza (Binomial):", varianza, "\n")
    cat("   Desviación estándar (Binomial):", desv_std, "\n")
    ```
...
-   c\. Si se inspeccionan 10 piezas independientemente, ¿cuál es la probabilidad de encontrar exactamente 2 defectuosas?
    ...
    ```{r}
    n_articulos <- 10
    n_inspeccion <- n_articulos
    n_piezas <- n_articulos
    k_defectuosas <- 2
    k_defectos <- 2:n_articulos

    prob_2_defectuosas <- dbinom(k_defectuosas, n_inspeccion, p_defectuosa)
    prob_2_defectos <- dbinom(2, n_piezas, p_defecto)
    prob_defectos <- dbinom(k_defectos, n_articulos, p_defecto)

    cat("c) Probabilidad de exactamente 2 defectuosas:\n")
    cat("   Usando p_defectuosa:", prob_2_defectuosas, "\n")
    cat("   Usando p_defecto:", round(prob_2_defectos, 6), "\n\n")
    ```
:::
...
-   a\. Modele como Bernoulli la respuesta a una pregunta
    ...
    ```{r}
    # Supongamos que una pregunta tiene 4 opciones y el estudiante responde al azar
    p_acierto <- 1 / 4
    p_error <- 1 - p_acierto

    # Distribución Bernoulli: 1 = acierto, 0 = error
    respuesta_bernoulli <- data.frame(
    ...
    kable(respuesta_bernoulli,
          caption = "Distribución Bernoulli - Respuesta al azar en pregunta de opción múltiple",
          align = "ccc") %>%
      kable_styling(bootstrap_options = "striped")
    ```
...
-   b\. ¿Cuál es la probabilidad de acertar?
    ...
    ```{r}
    # Probabilidad de acertar al responder al azar en una pregunta con 4 opciones
    p_acierto <- 1 / 4
    p_error <- 1 - p_acierto

    cat("a) Probabilidad de acertar una pregunta al azar:\n")
    cat("   P(acierto) =", p_acierto, "\n")
    cat("   P(error) =", p_error, "\n")
    ```
...
-   c\. Calcule los parámetros estadísticos
    ...
    ```{r}
    # Probabilidad de acertar al responder al azar
    p_acierto <- 1 / 4
    ...
    cat("b) Parámetros estadísticos (Bernoulli):\n")
    cat("   Media:", media, "\n")
...
-   a\) ¿Cuál es la probabilidad de obtener exactamente 8 caras?
    ...
    n_lanzamientos <- 15        # Número total de lanzamientos
    p_cara <- 0.5               # Probabilidad de obtener cara en cada lanzamiento
    k_caras <- 8                # Número exacto de caras deseadas

    # Cálculo de la probabilidad binomial
    prob_8_caras <- dbinom(k_caras, n_lanzamientos, p_cara)

    cat("a) Probabilidad de obtener exactamente 8 caras en 15 lanzamientos:", round(prob_8_caras, 6), "\n")
    ```
...
-   b\) ¿Cuál es la probabilidad de obtener al menos 12 caras?
    ...
    # Parámetros del experimento
    n_lanzamientos <- 15        # Número total de lanzamientos
    p_cara <- 0.5               # Probabilidad de obtener cara
    k_minimo <- 12              # Número mínimo de caras deseadas

    # Cálculo de la probabilidad acumulada para al menos 12 caras
    prob_al_menos_12 <- sum(dbinom(k_minimo:n_lanzamientos, n_lanzamientos, p_cara))

    cat("b) Probabilidad de obtener al menos 12 caras en 15 lanzamientos:", round(prob_al_menos_12, 6), "\n")
    ```
...
-   c\) Calcule la media y desviación estándar
    ...
    desv_std_binomial <- sqrt(varianza_binomial)

    cat("c) Parámetros estadísticos (Binomial):\n")
    cat("   Media:", media_binomial, "\n")
    cat("   Desviación estándar:", desv_std_binomial, "\n")
    ```
:::
...
-   a\) ¿Cuál es la probabilidad de que se curen exactamente 18 pacientes?
    ...
    ```{r}
    # Parámetros del experimento
    n_pacientes <- 20           # Número total de pacientes
    p_efectividad <- 0.85       # Probabilidad de que el medicamento funcione
    k_curados <- 18             # Número exacto de pacientes que se espera que se curen

    # Cálculo de la probabilidad binomial
    prob_18_curados <- dbinom(k_curados, n_pacientes, p_efectividad)

    cat("a) Probabilidad de que se curen exactamente 18 pacientes:", round(prob_18_curados, 6), "\n")
    ```
...
-   b\) ¿Cuál es la probabilidad de que se curen entre 15 y 19 pacientes?
    ...
    # Rango de pacientes curados
    k_curados_min <- 15
    k_curados_max <- 19

    # Cálculo de la probabilidad acumulada entre 15 y 19 curados
    prob_entre_15_19 <- sum(dbinom(k_curados_min:k_curados_max, n_pacientes, p_efectividad))

    cat("b) Probabilidad de que se curen entre 15 y 19 pacientes:", round(prob_entre_15_19, 6), "\n")
    ```
...
-   c\) ¿Cuántos pacientes se espera que se curen?
    ...
    # Cálculo de la media
    media_curados <- n_pacientes * p_efectividad

    cat("Cantidad esperada de pacientes curados:", media_curados, "\n")
    ```
:::
...
### Demostración teórica en R (estructurada)

```{r}
# Definimos dos variables binomiales independientes
n <- 10
m <- 15
p <- 0.3
...
# X ~ Bin(n, p)
# Y ~ Bin(m, p)
# Por definición: X + Y = Bin(n + m, p)
cat("Demostración teórica:\n")
cat("Si X ~ Bin(", n, ",", p, ") y Y ~ Bin(", m, ",", p, ") entonces X + Y ~ Bin(", n + m, ",", p, ")\n\n")
```
:::
...
Y <- rbinom(n_sim, m, p)
Z_simulado <- X + Y
Z_teorico <- rbinom(n_sim, n + m, p)

# Frecuencias relativas
tabla_sim <- table(Z_simulado) / n_sim
tabla_teorico <- table(Z_teorico) / n_sim

# Crear tabla comparativa
comparacion_bin <- data.frame(
  k = as.integer(names(tabla_sim)),
  Freq_Observada = as.numeric(tabla_sim),
  Prob_Teorica = as.numeric(tabla_teorico[names(tabla_sim)])
)
...
ggplot(comp_sim_long, aes(x = k, y = Probabilidad, fill = Tipo)) +
  geom_col(position = "dodge", alpha = 0.7) +
  scale_fill_manual(values = c("Teórica" = "green", "Simulada" = "pink")) +
  labs(title = paste("Comparación: Binomial(", n + m, ",", p, ") -", n_sim, "simulaciones"),
       subtitle = "Rosa: X + Y simulada | Verde: Binomial directa",
       x = "Número de Éxitos",
       y = "Probabilidad/Frecuencia",
       fill = "Distribución") +
  theme_minimal() +
