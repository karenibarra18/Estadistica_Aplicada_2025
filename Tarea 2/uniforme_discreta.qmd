...
# Números pares entre 1 y 8
bolas_pares <- sum((1:8) %% 2 == 0)

# Probabilidad de obtener un número par
prob_par <- bolas_pares / total_bolas

cat("Probabilidad de obtener un número par:", round(prob_par, 4), 
    "(", round(prob_par * 100, 2), "%)\n")
...
# Media de una distribución uniforme discreta
media <- (a + b) / 2
...
valores <- 0:4
n <- 500
simulacion <- sample(valores, size = n, replace = TRUE)

# Frecuencias observadas
frecuencia_observada <- table(simulacion)

# Frecuencia teórica (uniforme)
frecuencia_teorica <- rep(n / length(valores), length(valores))
names(frecuencia_teorica) <- valores

# Mostrar comparación
comparacion <- data.frame(
  Valor = valores,
  Observada = as.numeric(frecuencia_observada),
  Teorica = frecuencia_teorica
)
...
ggplot(comparacion, aes(x = factor(Valor))) +
  geom_bar(aes(y = Observada), stat = "identity", fill = "blue", alpha = 0.7) +
  geom_point(aes(y = Teorica), color = "red", size = 3) +
  geom_line(aes(y = Teorica, group = 1), color = "red", linetype = "dashed") +
  labs(title = "Frecuencia Observada vs Teórica",
       subtitle = "Distribución uniforme discreta en {0, 1, 2, 3, 4}",
       x = "Valor",
       y = "Frecuencia") +
  theme_minimal()
