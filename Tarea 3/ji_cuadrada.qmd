...
Para la distribución ji-cuadrada: $$F(x) = \int_0^x f(u) du = \int_0^x \frac{1}{\Gamma(n/2)} \left(\frac{1}{2}\right)^{n/2} u^{n/2-1} e^{-u/2} du$$
...
El coeficiente de variación (CV) mide la dispersión relativa y está dado por: $$CV = \frac{\sigma}{\mu} = \frac{\sqrt{2n}}{n} = \sqrt{\frac{2}{n}}$$
...
1.  Genera una gráfica de convergencia de la media muestral hacia la media teórica a medida que aumenta el tamaño de la muestra.
    ...
    df_resultados <- expand.grid(df = gl, tamaño = tamaños) %>%
      rowwise() %>%
      mutate(media_muestral = mean(rchisq(tamaño, df = df))) %>%
      ungroup()

    # Graficar con ggplot2
    ggplot(df_resultados, aes(x = tamaño, y = media_muestral, color = factor(df))) +
      geom_line(size = 1.2) +
      geom_hline(data = data.frame(df = gl, media_teorica = gl),
                 aes(yintercept = media_teorica, color = factor(df)),
                 linetype = "dashed", show.legend = FALSE) +
      facet_wrap(~df, scales = "free_y", ncol = 1,
                 labeller = label_bquote(n == .(df))) +
      labs(
        title = "Convergencia de la Media Muestral hacia la Media Teórica",
...
2.  Genera una gráfica de convergencia de la varianza muestral hacia la varianza teórica a medida que aumenta el tamaño de la muestra.
    ...
    # Simulación de varianzas muestrales
    df_varianzas <- expand.grid(df = gl, tamaño = tamaños) %>%
      rowwise() %>%
      mutate(varianza_muestral = var(rchisq(tamaño, df = df))) %>%
      ungroup()

    # Gráfico de convergencia de la varianza
    ggplot(df_varianzas, aes(x = tamaño, y = varianza_muestral, color = factor(df))) +
      geom_line(size = 1.2) +
      geom_hline(data = data.frame(df = gl, var_teo = 2 * gl),
                 aes(yintercept = var_teo, color = factor(df)),
                 linetype = "dashed", show.legend = FALSE) +
      facet_wrap(~df, scales = "free_y", ncol = 1,
                 labeller = label_bquote(n == .(df))) +
      labs(
        title = "Convergencia de la Varianza Muestral hacia la Varianza Teórica",
        x = "Tamaño de la Muestra",
        y = "Varianza Muestral",
        color = "Grados de Libertad"
      ) +
...
3.  Para el tamaño de muestra más grande, genera un histograma de la muestra junto con la función de densidad teórica superpuesta.
    ...
    set.seed(123)
    max_muestra <- max(tam_muestras)  # tamaño más grande de la secuencia
    grados_lib <- c(5, 20, 50)        # grados de libertad

    par(mfrow = c(3,1), mar = c(4,4,3,1))  # panel de 3 filas, márgenes ajustadas

    for (df_val in grados_lib) {
      
      # Generar muestra
      datos_muestra <- rchisq(max_muestra, df = df_val)
      
      # Histograma
      hist(datos_muestra, breaks = 30, probability = TRUE, 
           col = "skyblue2", border = "gray30",
           main = paste("Histograma df =", df_val, "con densidad teórica"),
           xlab = "Valores", ylab = "Densidad")
      
      # Densidad teórica superpuesta
      curve(dchisq(x, df = df_val), col = "darkred", lwd = 2, add = TRUE)
      
      # Leyenda
      legend("topright", legend = "Densidad Teórica", col = "darkred", lwd = 2, bty = "n")
    }
...
set.seed(456)
gl_tcl <- 10
tamanos <- c(5, 10, 30, 50, 100, 500)
par(mfrow = c(3,2), mar = c(4,4,3,1))

for (tam in tamanos) {
  medias <- replicate(1000, mean(rchisq(tam, df = gl_tcl)))
  
  hist(medias, breaks = 30, probability = TRUE, col = "skyblue3",
       main = paste("TCL para tamaño =", tam, ", df =", gl_tcl),
       xlab = "Media Muestral", ylab = "Densidad")
  
  mu_teor <- gl_tcl
  sigma_teor <- sqrt(2 * gl_tcl / tam)
  curve(dnorm(x, mean = mu_teor, sd = sigma_teor), col = "darkred", lwd = 2, add = TRUE)
  
  legend("topright", legend = "Normal Teórica", col = "darkred", lwd = 2, bty = "n")
}
...
simulaciones <- 10000
df_val <- 10
sumas <- replicate(simulaciones, sum(rnorm(df_val)^2))
df_sumas <- data.frame(valor = sumas)

ggplot(df_sumas, aes(x = valor)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightgreen", color = "black", alpha = 0.7) +
  geom_function(fun = function(x) dchisq(x, df = df_val), aes(color = "Chi-cuadrada Teórica"), size = 1.2) +
  scale_color_manual(values = c("Chi-cuadrada Teórica" = "magenta")) +
  labs(title = "Suma de variables normales estándar al cuadrado",
       subtitle = "Verificación de aditividad de la distribución chi-cuadrada",
       x = "Valor", y = "Densidad") +
  theme_minimal() +
...
for (df_val in dfs) {
  datos <- rchisq(simulaciones, df = df_val)
  
  hist(datos, breaks = 30, probability = TRUE, col = "lightyellow",
       main = paste("Aproximación normal para df =", df_val),
       xlab = "x", ylab = "Densidad")
  
  densidad <- density(datos)
  lines(densidad, col = "darkgreen", lwd = 2)
  
  mu_teor <- df_val
  sigma_teor <- sqrt(2 * df_val)
  curve(dnorm(x, mean = mu_teor, sd = sigma_teor), col = "darkblue", lwd = 2, add = TRUE)
  
  legend("topright", legend = c("Distribución muestral", "Normal Teórica"),
         col = c("darkgreen", "darkblue"), lty = c(1,2), lwd = c(1,2))
}
...
muestra1 <- rchisq(n_sim, df = df1)
muestra2 <- rchisq(n_sim, df = df2)
suma <- muestra1 + muestra2
df_suma <- data.frame(valor = suma)

ggplot(df_suma, aes(x = valor)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "pink", color = "black", alpha = 0.7) +
  geom_function(fun = function(x) dchisq(x, df = df1 + df2), aes(color = "Chi-cuadrada Teórica"), size = 1.2) +
  scale_color_manual(values = c("Chi-cuadrada Teórica" = "darkblue")) +
  labs(title = paste("Suma de chi-cuadradas independientes (df1 =", df1, ", df2 =", df2, ")"),
       subtitle = "Verificación de aditividad",
       x = "Valor", y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        plot.subtitle = element_text(hjust = 0.5, size = 12),
