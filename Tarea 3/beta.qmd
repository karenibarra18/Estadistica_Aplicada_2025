...
3.  Para el tamaño de muestra más grande, genera un histograma de la muestra junto con la curva de la PDF teórica superpuesta.
    ...
    medias <- numeric(length(n_seq))
    varianzas <- numeric(length(n_seq))

    # Simulación
    for (i in seq_along(n_seq)) {
      muestra <- rbeta(n_seq[i], a, b)
    ...
    p1 <- ggplot(df, aes(x = tamaño, y = media_muestral)) +
      geom_line(color = "steelblue", size = 1.2) +
      geom_hline(yintercept = media_teorica, linetype = "dashed", color = "pink") +
      labs(
        title = "Convergencia de la Media Muestral",
        subtitle = paste("Media Teórica =", round(media_teorica, 4)),
    ...
    p2 <- ggplot(df, aes(x = tamaño, y = varianza_muestral)) +
      geom_line(color = "violet", size = 1.2) +
      geom_hline(yintercept = varianza_teorica, linetype = "dashed", color = "lightblue") +
      labs(
        title = "Convergencia de la Varianza Muestral",
        subtitle = paste("Varianza Teórica =", round(varianza_teorica, 4)),
        x = "Tamaño de la Muestra",
        y = "Varianza Muestral"
      ) +
      theme_minimal()
    ...
    n_max <- max(n_seq)
    muestra_grande <- rbeta(n_max, a, b)
    df_hist <- data.frame(x = muestra_grande)

    ggplot(df_hist, aes(x = x)) +
      geom_histogram(aes(y = ..density..), bins = 30, fill = "lightgreen", color = "blue", alpha = 0.7) +
      stat_function(fun = dbeta, args = list(shape1 = a, shape2 = b), color = "purple", size = 1.2) +
      labs(
        title = paste("Histograma de la Muestra (n =", n_max, ") con PDF Teórica"),
        x = "x",
        y = "Densidad"
      ) +
      theme_minimal() +
      annotate("text", x = 0.7, y = max(density(muestra_grande)$y) * 0.8,
               label = "Curva Roja: PDF Teórica", color = "darkblue", fontface = "bold")
...
b <- 5
n_replicas <- 1000
tamaños_muestra <- c(5, 30, 100, 500)

# Función para generar medias muestrales
generar_medias <- function(n, a, b, rep) {
  replicate(rep, mean(rbeta(n, a, b)))
}

# Función para obtener resumen estadístico
resumen_tcl <- function(medias, n, a, b) {
  media <- mean(medias)
  varianza <- var(medias)
  sd <- sd(medias)
  media_teo <- a / (a + b)
  varianza_teo <- (a * b) / ((a + b)^2 * (a + b + 1)) / n
  ...
  cat(sprintf("Desviación Estándar: %.4f | Desv. Est. Teórica: %.4f\n\n", sd, sd_teo))
}

# Generación de gráficos y resumen
plots_tcl <- list()
cat("RESUMEN DE LA VERIFICACIÓN DEL TEOREMA CENTRAL DEL LÍMITE\n")

for (n in tamaños_muestra) {
  medias <- generar_medias(n, a, b, n_replicas)
  df <- data.frame(medias = medias)
  media_teo <- a / (a + b)
  sd_teo <- sqrt((a * b) / ((a + b)^2 * (a + b + 1)) / n)
  
  # Gráfico
  p <- ggplot(df, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "violet", color = "blue", alpha = 0.7) +
    stat_function(fun = dnorm, args = list(mean = media_teo, sd = sd_teo), color = "darkgray", size = 1.2) +
    labs(
      title = paste("TCL: n =", n),
