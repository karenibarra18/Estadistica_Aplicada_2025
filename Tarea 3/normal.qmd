...
2.  ¿Qué porcentaje del tiempo la temperatura está fuera de especificación?
    ...
    # Parámetros del proceso
    mu <- 350; sigma <- 5
    lim_inf <- 340; lim_sup <- 360
    n_mediciones <- 1000

    # Simulación de temperaturas
    set.seed(123)  # Semilla para reproducibilidad (opcional)
    temperaturas <- rnorm(n = n_mediciones, mean = mu, sd = sigma)

    # Cálculo de fuera de especificación
    fuera_rango <- temperaturas < lim_inf | temperaturas > lim_sup
    t_fuera_spec <- sum(fuera_rango)
    pc_fuera <- 100 * t_fuera_spec / n_mediciones

    # Resultado
    cat(sprintf("El porcentaje de tiempo donde la temperatura está fuera de especificación es: %.2f %%\n", pc_fuera))
...
3.  Si el proceso se descentra a $\mu = 355°C$, ¿cómo cambia la calidad?
    ...
    ```{r}
    #| code-fold: true
    # Evaluación de calidad con proceso descentrado

    mu2 <- 355
    temperaturas_2 <- rnorm(n = n_mediciones, mean = mu2, sd = sigma)

    fuera_rango_2 <- temperaturas_2 < lim_inf | temperaturas_2 > lim_sup
    t_fuera_spec_2 <- sum(fuera_rango_2)
    pc_fuera_2 <- 100 * t_fuera_spec_2 / n_mediciones

    cat(sprintf("El porcentaje de tiempo donde la temperatura está fuera de especificación con μ=355°C es: %.2f %%\n", pc_fuera_2))
    ```
...
4.  ¿Qué $\sigma$ se necesita para tener 99.9% de productos dentro de especificación?
    ...
    # Cálculo de sigma para 99.9% dentro de especificación

    z <- qnorm(0.999)
    rango_superior <- lim_sup - mu
    sigma_necesario <- rango_superior / z

    cat(sprintf("El σ necesario para tener 99.9%% de productos dentro de especificación es: %.2f °C\n", sigma_necesario))
...
2.  ¿Cuál es la probabilidad de tener ventas superiores a 60,000 pesos?
    ...
    ```{r}
    #| code-fold: true
    # Simulación de ventas mensuales y cálculo de probabilidad

    mu <- 50000; sigma <- 12000; n_meses <- 24
    ventas <- rnorm(n = n_meses, mean = mu, sd = sigma)

    umbral <- 60000
    pc_mas_60000 <- 100 * (1 - pnorm(umbral, mean = mu, sd = sigma))

    cat(sprintf("La probabilidad de tener ventas superiores a 60,000 pesos es: %.2f %%\n", pc_mas_60000))
    ```
...
3.  ¿En cuántos meses se espera tener ventas por debajo de 30,000 pesos?
    ...
    ```{r}
    #| code-fold: true
    # Probabilidad de ventas menores a 30,000 pesos

    prob_menos_30k <- pnorm(30000, mean = mu, sd = sigma)
    ventas_menos_30000 <- prob_menos_30k * n_meses

    cat(sprintf("Se espera tener ventas por debajo de 30,000 pesos en: %.2f meses\n", ventas_menos_30000))
    ```
...
4.  Calcula el rango intercuartílico de las ventas
    ...
    cuartiles <- quantile(ventas, probs = c(0.25, 0.75))
    iqr <- diff(cuartiles)

    cat(sprintf("El rango intercuartílico de las ventas es: %.2f pesos\n", iqr))
...
5.  Si la meta es superar 45,000 pesos, ¿qué porcentaje de meses se cumple?
    ...
    ```{r}
    #| code-fold: true
    # Evaluación del cumplimiento de la meta de ventas

    meta <- 45000
    cumplimiento_meta <- ventas > meta
    pc_mas_45000 <- 100 * mean(cumplimiento_meta)

    cat(sprintf("El porcentaje de meses que se cumple la meta de superar 45,000 pesos es: %.2f %%\n", pc_mas_45000))
    ```
:::
...
2.  ¿Qué garantía (en horas) cubre al 95% de las baterías?
    ...
    ```{r}
    #| code-fold: true
    # Simulación de vida útil de baterías y cálculo de garantía

    mu <- 1200; sigma <- 150; n_baterias <- 800
    vida_util <- rnorm(n = n_baterias, mean = mu, sd = sigma)

    nivel_confianza <- 0.95
    garantia_h_95 <- qnorm(1 - nivel_confianza, mean = mu, sd = sigma)

    cat(sprintf("La garantía que cubre al 95%% de las baterías es de: %.2f horas\n", garantia_h_95))
    ```
...
3.  Si se venden 10,000 baterías con garantía de 900 horas, ¿cuántas se espera que fallen?
    ...
    ```{r}
    #| code-fold: true
    # Estimación de fallas bajo garantía de 900 horas

    n_vendidas <- 10000
    umbral_garantia <- 900
    pc_falla_900 <- pnorm(umbral_garantia, mean = mu, sd = sigma)
    n_fallas_esperadas <- pc_falla_900 * n_vendidas

    cat(sprintf("Se espera que fallen: %.2f baterías con garantía de 900 horas\n", n_fallas_esperadas))
    ```
...
4.  Compara la media y mediana de tu simulación con los valores teóricos
    ...
    ```{r}
    #| code-fold: true
    # Comparación de estadísticos simulados y teóricos

    media_sim <- mean(vida_util)
    mediana_sim <- median(vida_util)

    cat(sprintf("Media simulada: %.2f horas\n", media_sim))
    cat(sprintf("Mediana simulada: %.2f horas\n", mediana_sim))
    cat(sprintf("Media teórica: %.0f horas\n", mu))
    cat(sprintf("Mediana teórica: %.0f horas\n", mu))
...
5.  ¿Cuál es el percentil 10 de la distribución?
    ...
    ```{r}
    #| code-fold: true
    # Cálculo del percentil 10 de la duración de baterías

    nivel_percentil <- 0.10
    percentil_10 <- qnorm(nivel_percentil, mean = mu, sd = sigma)

    cat(sprintf("El percentil 10 de la distribución es: %.2f horas\n", percentil_10))
    ```
:::
...
2.  ¿Qué porcentaje de errores está entre -5 y +5 unidades?
    ...
    ```{r}
    #| code-fold: true
    # Simulación de errores de medición y cálculo de porcentaje dentro del rango

    mu <- 0; sigma <- 2.5; n_mediciones <- 1000
    errores <- rnorm(n = n_mediciones, mean = mu, sd = sigma)

    rango_valido <- errores > -5 & errores < 5
    pc_menos5_mas5 <- 100 * mean(rango_valido)

    cat(sprintf("El porcentaje de errores que está entre -5 y +5 unidades es: %.2f %%\n", pc_menos5_mas5))
    ```
...
3.  Calcula la probabilidad de que un error sea mayor a 6 unidades en valor absoluto
    ...
    ```{r}
    #| code-fold: true
    # Cálculo de probabilidad de errores extremos

    umbral_extremo <- 6
    pc_mas_6 <- 2 * pnorm(-umbral_extremo, mean = mu, sd = sigma)

    cat(sprintf("La probabilidad de que un error sea mayor a 6 unidades en valor absoluto es: %.2f %%\n", pc_mas_6 * 100))
...
4.  Si se considera "preciso" un error ≤ 3 unidades, ¿qué porcentaje de mediciones son precisas?
    ...
    ```{r}
    #| code-fold: true
    # Evaluación de precisión de mediciones

    umbral_precision <- 3
    pc_preciso <- 100 * mean(abs(errores) <= umbral_precision)

    cat(sprintf("El porcentaje de mediciones que son precisas (error ≤ 3 unidades) es: %.2f %%\n", pc_preciso))
    ```
...
5.  Verifica que la distribución es simétrica alrededor de 0
    ...
    ```{r}
    #| code-fold: true
    # Verificación de simetría en la distribución de errores

    media_errores <- mean(errores)
    desv_errores <- sd(errores)
    asimetria <- mean((errores - media_errores)^3) / (desv_errores^3)

    cat(sprintf("El coeficiente de asimetría de la distribución es: %.4f\n", asimetria))
...
2.  ¿Qué porcentaje de estudiantes del Grupo A supera la media del Grupo B?
    ...
    grupo_A <- rnorm(n = n_estudiantes, mean = mu_A, sd = sigma_A)
    grupo_B <- rnorm(n = n_estudiantes, mean = mu_B, sd = sigma_B)

    superan_media_B <- grupo_A > mu_B
    pc_A_mas_B <- 100 * mean(superan_media_B)

    cat(sprintf("El porcentaje de estudiantes del Grupo A que supera la media del Grupo B es: %.2f %%\n", pc_A_mas_B))
    ```
...
3.  ¿Cuál es la probabilidad de que un estudiante del Grupo B tenga puntuación menor a 70?
    ...
    umbral <- 70
    pc_B_menos_70 <- 100 * pnorm(umbral, mean = mu_B, sd = sigma_B)

    cat(sprintf("La probabilidad de que un estudiante del Grupo B tenga puntuación menor a 70 es: %.2f %%\n", pc_B_menos_70))
    ```
...
4.  Compara las distribuciones visualmente
    ...
    df_examen <- data.frame(
      puntuacion = c(grupo_A, grupo_B),
      grupo = factor(rep(c("A", "B"), each = n_estudiantes))
    )

    ggplot(df_examen, aes(x = puntuacion, fill = grupo)) +
      geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.6, position = "identity") +
      stat_function(fun = dnorm, args = list(mean = mu_A, sd = sigma_A), color = "violet", size = 1) +
      stat_function(fun = dnorm, args = list(mean = mu_B, sd = sigma_B), color = "hotpink4", size = 1) +
...
5.  ¿A partir de qué puntuación está el 10% superior de cada grupo?
    ...
    cat(sprintf("El 10%% superior del Grupo A tiene puntuación mayor a: %.2f\n", p90_A))
    cat(sprintf("El 10%% superior del Grupo B tiene puntuación mayor a: %.2f\n", p90_B))
