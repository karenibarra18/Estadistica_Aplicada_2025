...
Antes de estudiar la distribución gamma, es fundamental comprender la
**función gamma**, que es la base matemática de esta distribución y una
generalización del factorial para los números reales.
...
Esta integral impropia converge para todos los valores reales
$\alpha > 0$ y para todos los números complejos con parte real positiva.
:::
...
La **distribución gamma** es una familia de distribuciones continuas
extremadamente versátil que surge naturalmente en múltiples contextos
estadísticos y aplicados. Su importancia radica en su capacidad para
modelar una amplia variedad de fenómenos que involucran **tiempos de
espera**, **sumas de variables aleatorias** y **procesos de llegada**.

La distribución gamma es fundamental para modelar:

-   **Tiempo hasta la k-ésima falla** en sistemas con múltiples
    componentes
...
::: {#def-dist_gamma}
Una variable aleatoria continua $X$ sigue una **distribución gamma** si
...
A una variable aleatoria con distribución gamma la denotamos como
...
La distribución gamma incluye varias distribuciones conocidas como casos
especiales:
...
-   $\alpha < 1$: Distribución en forma de "J" invertida, altamente
    sesgada
...
-   $\lambda$ pequeño: Distribución más dispersa, valores más grandes
    probables
-   $\lambda$ grande: Distribución más concentrada, valores más pequeños
    probables
-   **Efecto de escala**: $\lambda$ actúa como un factor de escala
    inverso
...
La función de densidad de probabilidad de la distribución gamma puede
escribirse como:
...
El comportamiento de $f(x)$ cuando $x \to 0^+$ depende crucialmente del
parámetro de forma $\alpha$:
...
-   **Si α = 1**:
    $\lim_{x \to 0} f(x) = \frac{\lambda}{\Gamma(1)} = \lambda$
...
-   Para $x > \frac{\alpha-1}{\lambda}$: $f'(x) < 0$ (función
    decreciente)
...
El decaimiento es **exponencial**, más rápido que cualquier función
polinomial.

## Propiedades de la Función de Distribución (CDF)

::: {#def-cdf_gamma}
La función de distribución acumulativa (CDF) de la distribución gamma se
define como:
...
No existe una forma cerrada para la función cuantil gamma, por lo que se
calculan numéricamente.
...
Para calcular probabilidades en intervalos, utilizamos la propiedad
fundamental:
...
La media de una distribución gamma
$X \sim \text{gamma}(\alpha, \lambda)$ se calcula como:
...
Para resolver esta integral, utilizamos la sustitución $u = \lambda x$,
donde $du = \lambda dx$ y $x = u/\lambda$:
...
Reconocemos que
$\int_{0}^{\infty} u^\alpha e^{-u} du = \Gamma(\alpha + 1)$, y usando la
...
-   La media es **directamente proporcional** al parámetro de forma
    $\alpha$
-   La media es **inversamente proporcional** al parámetro de tasa
    $\lambda$
-   Si $\alpha$ aumenta (manteniendo $\lambda$ fijo), la distribución se
    desplaza hacia la derecha
-   Si $\lambda$ aumenta (manteniendo $\alpha$ fijo), la distribución se
    concentra hacia valores menores
...
Usando
$\Gamma(\alpha + 2) = (\alpha + 1) \cdot \alpha \cdot \Gamma(\alpha)$:
...
-   La varianza es **inversamente proporcional** al cuadrado del
    parámetro de tasa $\lambda$
-   Mayor $\alpha$ implica mayor dispersión (para $\lambda$ fijo)
-   Mayor $\lambda$ implica menor dispersión (concentración hacia
    valores menores)
...
-   El coeficiente de variación **solo depende del parámetro de forma**
    $\alpha$
...
En esta sección generaremos muestras aleatorias de la distribución gamma
y compararemos los estadísticos muestrales con los parámetros teóricos
conocidos.

**Parámetros de la Distribución**

Consideremos una distribución $\text{gamma}(\alpha = 3, \lambda = 1.5)$
para nuestras simulaciones:
...
**Simulación con diferentes tamaños de muestra:** Convergencia de
...
-   Genera 1000 muestras de tamaños $n= 5, 10, 20, 30, 50, 100$ de una
    distribución $\text{gamma}(\alpha = 2.5, \lambda = 1.8)$.
    ...
    # Parámetros de la distribución gamma
    alpha <- 2.5      # parámetro de forma (α)
    lambda <- 1.8     # parámetro de tasa (λ)

    # Tamaños de muestra
    n_valores <- c(5, 10, 20, 30, 50, 100)

    # Lista para almacenar las muestras generadas
    muestras_gamma <- list()

    # Generar las 1000 muestras para cada tamaño n
    for (n in n_valores) {
      muestras_gamma[[paste0("n_", n)]] <- replicate(
        1000,
        rgamma(n, shape = alpha, rate = lambda)
      )
    }

    # Mostrar un ejemplo (primeras observaciones de n = 10)
    head(muestras_gamma$n_10[, 1:3])

    ```

-   Calcula la media muestral para cada muestra y compárala con la media
    teórica.

    ```{r}
    #| code-fold: true

    # Calcular la media teórica de la distribución Gamma
    media_teorica <- alpha / lambda

    # Crear un data frame vacío para guardar los resultados
    comparacion_medias <- data.frame()

    # Calcular la media muestral promedio de las 1000 simulaciones para cada n
    for (n in n_valores) {
      # Obtener las 1000 muestras de tamaño n
      muestras_n <- muestras_gamma[[paste0("n_", n)]]
      
      # Calcular la media de cada muestra
      medias_muestrales <- apply(muestras_n, 2, mean)
      
      # Guardar resultados en el data frame
      comparacion_medias <- rbind(comparacion_medias, data.frame(
        n = n,
        media_muestral_promedio = mean(medias_muestrales),
    ...
    kable(comparacion_medias,
          caption = "Comparación entre la Media Muestral Promedio y la Media Teórica",
          digits = 4,
          col.names = c("Tamaño de muestra (n)", "Media Muestral Promedio", "Media Teórica")) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

    ```

-   Calcula la varianza de las medias muestrales y compárala con la
    varianza teórica dividida por $n$.

    ```{r}
    #| code-fold: true

    # Varianza teórica de la distribución Gamma: Var(X) = α / λ²
    var_teorica <- alpha / (lambda^2)

    # Crear un nuevo data frame para almacenar resultados
    comparacion_varianzas <- data.frame()

    # Calcular la varianza muestral de las medias para cada tamaño n
    for (n in n_valores) {
      # Obtener las 1000 muestras
      muestras_n <- muestras_gamma[[paste0("n_", n)]]
      
      # Calcular la media de cada muestra
      medias_muestrales <- apply(muestras_n, 2, mean)
      
      # Calcular varianza teórica ajustada (Var(X̄) = Var(X) / n)
      var_teorica_ajustada <- var_teorica / n
      
      # Guardar resultados
      comparacion_varianzas <- rbind(comparacion_varianzas, data.frame(
        n = n,
        varianza_muestral = var(medias_muestrales),
        varianza_teorica_ajustada = var_teorica_ajustada
      ))
    }

    # Mostrar la comparación en una tabla
    kable(comparacion_varianzas,
          caption = "Comparación entre la Varianza de las Medias Muestrales y la Varianza Teórica / n",
          digits = 6,
          col.names = c("Tamaño de muestra (n)", "Varianza Muestral", "Varianza Teórica / n")) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

    ```

-   Grafica los histogramas de las medias muestrales y superpone la
    distribución normal teórica según el TCL.

    ```{r}
    #| code-fold: true
    #| fig.width: 12

    # Configuración del espacio para mostrar múltiples histogramas
    par(mfrow = c(2, 3))

    # Bucle para generar las gráficas según el tamaño de muestra
    for (tam in muestras_n) {
      
      # Generar 1000 medias muestrales para cada tamaño n
      medias_muestra <- replicate(1000, mean(rgamma(tam, shape = alpha_tcl, rate = lambda_tcl)))
      
      # Calcular parámetros teóricos de la distribución normal aproximada
      mu_teorico <- alpha_tcl / lambda_tcl
      desv_teorica <- sqrt((alpha_tcl / (lambda_tcl^2)) / tam)
      
      # Crear histograma de las medias muestrales
      hist(medias_muestra, breaks = 30, probability = TRUE,
           main = paste("Tamaño de muestra =", tam),
           xlab = "Media muestral",
           col = "lightcyan3", border = "gray40")
      
      # Superponer la curva normal teórica
      curve(dnorm(x, mean = mu_teorico, sd = desv_teorica),
            col = "darkred", lwd = 2, add = TRUE)
      
      # Agregar una leyenda descriptiva
      legend("topright", legend = c("Distribución Normal Teórica"),
...
**Proposición** Sean $X_1, \dots, X_n$ variables aleatorias
independientes e idénticamente distribuidas con distribución exponencial
$\text{exp}(\lambda)$. Entonces, la suma $Y = \sum_{i=1}^{n} X_i$ sigue
una distribución gamma $\text{gamma}(n, \lambda)$.

-   Simula 1000 sumas de $n=5$ variables aleatorias
    $\text{exp}(\lambda = 1.5)$.

-   Grafica el histograma de las sumas y superpone la densidad teórica
    de $\text{gamma}(5, 1.5)$.
    ...
    # Generar 1000 sumas de 5 variables Exp(1.5)
    sumas_exponenciales <- replicate(num_simulaciones, sum(rexp(num_variables, rate = lambda_exp)))

    # Histograma de las sumas
    hist(sumas_exponenciales, breaks = 30, probability = TRUE,
         main = paste("Suma de", num_variables, "variables Exp(λ =", lambda_exp, ")"),
         xlab = "Suma de variables aleatorias",
         col = "lightseagreen", border = "gray30")

    # Superponer densidad teórica Gamma
    curve(dgamma(x, shape = num_variables, rate = lambda_exp), 
          col = "darkred", lwd = 2, add = TRUE)

    # Leyenda
    legend("topright", legend = paste("Densidad teórica Gamma(", num_variables, ",", lambda_exp, ")", sep=""),
           col = "darkred", lwd = 2, bty = "n")

    ```

-   Repite el ejercicio para $n=10$ y $n=20$ y observa cómo cambia la
    ...
    for (n_val in tamaños) {
      
      # Simular sumas de n_val variables exponenciales
      sumas_sim <- replicate(num_simulaciones, sum(rexp(n_val, rate = lambda_exp)))
      
      # Histograma
      hist(sumas_sim, breaks = 30, probability = TRUE,
           main = paste("Suma de", n_val, "variables Exp(λ =", lambda_exp, ")"),
           xlab = "Suma de variables aleatorias",
           col = "lightcoral", border = "gray30")
      
      # Superponer densidad teórica Gamma(n, lambda)
      curve(dgamma(x, shape = n_val, rate = lambda_exp), 
            col = "darkblue", lwd = 2, add = TRUE)
      
      # Leyenda
      legend("topright", legend = paste("Densidad teórica Gamma(", n_val, ",", lambda_exp, ")", sep=""),
